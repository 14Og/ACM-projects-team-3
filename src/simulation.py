"""Closed-loop simulation for robust manipulator control."""
...
from .controller import ReferenceGenerator
from .system import JointSpacePlant, PlanarArm
...
def run_rollout(config: ProjectConfig, controller: Controller) -> Rollout:
    ...
    plant = JointSpacePlant(config.robot.initial_angles, config.robot, config.dynamics)
    controller.reset()
    ...
    q_model = np.zeros((n_steps, n_joints))
    dq_model = np.zeros((n_steps, n_joints))
    ddq_model = np.zeros((n_steps, n_joints))
    ...
    for index, t in enumerate(time):
        ...
        q_des[index] = ref.q
        dq_des[index] = ref.dq
        q_model[index] = info.q_model
        dq_model[index] = info.dq_model
        ddq_model[index] = info.ddq_model
...
def save_rollout_data_csv(rollouts: dict[str, Rollout], path: str | Path) -> Path:
    ...
    joint_fields = [
        ...
        *(f"q_m{j + 1}" for j in range(n_joints)),
        *(f"dq_m{j + 1}" for j in range(n_joints)),
        *(f"ddq_m{j + 1}" for j in range(n_joints)),
    ...
    with output_path.open("w", newline="", encoding="utf-8") as file:
        ...
        for label, rollout in rollouts.items():
            for index, time in enumerate(rollout.time):
                ...
                row.update(_joint_row("q_m", rollout.q_model[index]))
                row.update(_joint_row("dq_m", rollout.dq_model[index]))
                row.update(_joint_row("ddq_m", rollout.ddq_model[index]))
